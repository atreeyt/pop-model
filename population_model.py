...
class PopulationModel:
    def __init__(self, chromosome_dict={"RR", "Rr", "rR", "rr"}):
        ...
        """
        self.seed_pop = seed_population.SeedPopulation(
    ...
    def add_seeds(self, chromosome, count) -> None:
        ...
        self.seed_pop.update_counts(
            count_dict, replace=False, remove_others=False
        )
        return
    ...
    def remove_seeds(self, chromosome, count) -> None:
        ...
        if count > population:
            logger.warning(
                f"Attempted to remove more seeds than currently in"
    ...
    def purge_population(
        ...
        if amount_to_remove < 0:
            logging.warning(
                "Attempted population purge with negative value, ignoring."
            )
            return
        # Assume percentage.
    ...
    def get_population_change(self) -> dict:
        ...
        for chromosome1, frequency1 in frequencies.items():
            for chromosome2, frequency2 in frequencies.items():
                children = self._get_chromosome_pairing_output(
                    chromosome1, chromosome2
                )
                logger.debug(
                    f"{chromosome1}, {chromosome2} children: {children}"
                )
                # Division by 0.25 as there is there are four options for
                # the children (e.g. RR, Rr, rR, and rr).
                new_seed = (
                    sum(self.seed_pop.get_population().values())
                    * frequency1
                    * frequency2
                    * 0.25
                )
